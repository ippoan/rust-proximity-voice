...
namespace Oxide.Plugins
{
    ...
    public class ProximityVoice : RustPlugin
    {
        ...
        public class PluginConfig
        {
            ...
            ///
            /// **既定 0** = 仕様どおり「変化が無ければ送らない」で、静止中のトラフィックはゼロ。
            /// ただしリレーが WS 接続時に保持済みの graph / yaw を撒き直さない実装だと、
            /// 途中から繋いだ PWA が「誰も動かない限り何も受け取れない」状態になる。
            /// その場合はここを 1 にする (1 Hz なら実質ただの keepalive)。
            public float resend_unchanged_s = 0f;
