...
namespace Oxide.Plugins
{
    ...
    public class ProximityVoice : RustPlugin
    {
        ...
        public class PluginConfig
        {
            ...
            ///
            /// **リレーだけが再起動したときの復旧のために要る。** プラグインは走り続けている
            /// ので、名簿は 2 秒ごとに無条件で飛んで復旧する。しかし graph は変化時のみなので、
            /// リレー側の `hears` は空のまま埋まらない。ブラウザは再接続して `resync` を
            /// 受け取るが、**リレーが持っている中身が空**なので、誰かが動くまで無音になる。
            /// 人が動き続けていればすぐ直るが、全員が拠点で静止していると止まったままになる。
            ///
            /// 既定 5 秒の代償は小さい。10 人で 60 バイト × 10 ÷ 5 秒 ≒ **120 B/s**。
            /// 「変化が無ければ送らない」の意図 (静止中に 2 Hz で流し続けない) は保たれる。
            ///
            /// 0 にすると完全な変化検出になり、静止中のトラフィックはゼロになる。
            public float resend_unchanged_s = 5f;
