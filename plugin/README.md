...
| `hud_hz` | `3` | HUD の更新レート |
| `resend_unchanged_s` | `30` | 変化が無くても graph / yaw を再送する間隔 (秒)。0 で無効。**下の説明を読む。短くしてはいけない** |
| `http_timeout_s` | `2` | HTTP のタイムアウト |

### `resend_unchanged_s` — 自己修復しない無音を潰す保険。**短くしてはいけない**

graph は仕様どおり「変化が無ければ送らない」(`docs/protocol.md` の
「静止中は graph が飛ばない」)。この設定はその原則を曲げるものではなく、
**放っておいても直らない無音**だけを潰すためにある。

効くのは 3 つの場面:

1. リレーが古くて WS 接続時の撒き直しを持たない
2. 独自のリレーと繋ぐ
3. **リレーだけが再起動した** ← 本命

3 番を追う。プラグインは走り続けている。

リレーが再起動すると、名簿は 2 秒で戻るが **graph の状態は空のまま**になる。
ブラウザは再接続して `resync` も受けるが、リレーに撒くものが無い。
全員が静止していると、プラグインが再送しない限り誰も埋められない。

| | |
|---|---|
| 名簿 | `roster` は 2 秒ごとに**無条件で**飛ぶので 2 秒で復旧する |
| 可聴グラフ | `graph` は**変化時のみ**なので、リレー側の `hears` は空のまま埋まらない |
| ブラウザ | WS が `bye` 無しで切れると `signal.js` が指数バックオフで再接続し、`ready` で `releaseAll()` → 張り直し → リレーが `resync` を撒く。**接続も `resync` も起きる** |
| 結果 | それでも **リレーに撒くものが無い** ので、誰かが動くまで無音 |

**ここが要点: リレーの状態を埋め直せるのはプラグインだけ。** 無音になる理由は
「クライアントが戻ってこないから」ではないので、クライアント側では直せない。

Rust のサーバーなら人は動き続けるので実際にはすぐ復旧するが、
**全員が拠点で静止している場面では止まったまま**になる。自己修復しない。

#### なぜ 30 秒なのか — 短くすると秒未満の競合を隠す

**この再送は、リレーや PWA 側の「1 通取りこぼすと次が来ない」たぐいのバグを覆い隠す。**

取りこぼしは本来 **「永久に無音」** として現れるから見つかる。もし数秒の再送が
既定で効いていると、同じバグは **「入室時に数秒だけ無音」** になり、運用では
「ちょっと出遅れた」に見えて報告されない。静止させて push を止める種類の
試験でしか捕まらなくなる。**直したのに既定で覆いを掛ける**ことになり、
次に同じ形が入っても誰も気づかない。
...
| | |
|---|---|
| 自己修復しない無音 (上の 3 場面) | 最大 30 秒で復旧する。実際に 30 秒待つのは**全員が完全静止**しているときだけ |
| 秒未満の競合を踏んだバグ | **「最大 30 秒無音」として現れる**。これは報告される長さ |

「無音が目立つ」という性質を保ったまま、自己修復しない経路だけを潰す、という線引き。
**30 秒なのは、これより短いと秒未満の競合 (`graph` → `peer` の到着順など) を隠すため。**
「最大 30 秒無音」は報告される長さだが、**`5` 秒は「接続が遅い」に見えて報告されない。
戻さないこと。**
...
> これは WS 接続時の撒き直しが無かったこと (issue #11) の回避策**ではない**。
> #11 はリレー側の `resync` で直っており (PR #14)、途中参加のブラウザは
> `hears_of` / `yaw_of` の中身を接続時に受け取る。上に書いたのは、
> **その撒き直しが返す中身自体が空になる**という別の話。
...
| `POST /internal/graph` | 2 Hz、**変化時のみ** (+ `resend_unchanged_s` ごとに 1 回) | 聞き手ごとの `{ id, d, b, sub }`。**距離の近い順** |
| `POST /internal/yaw` | 20 Hz、変化時のみ (+ `resend_unchanged_s` ごとに 1 回) | `[[SteamID, 度], ...]` を 1 リクエストに全員ぶん |
