...
| `hud_hz` | `3` | HUD の更新レート |
| `resend_unchanged_s` | `5` | 変化が無くても graph / yaw を再送する間隔 (秒)。0 で無効。**下の説明を読む** |
| `http_timeout_s` | `2` | HTTP のタイムアウト |

### `resend_unchanged_s` — リレー再起動からの復旧のために要る

graph は仕様どおり「変化が無ければ送らない」(`docs/protocol.md` の
「静止中は graph が飛ばない」)。この設定は**その原則を曲げるものではなく、
リレーだけが再起動したときに状態を埋め直すためにある。**

**リレーだけが落ちて上がり直した場面**を考える。プラグインは走り続けている。

| | |
|---|---|
| 名簿 | `roster` は 2 秒ごとに**無条件で**飛ぶので 2 秒で復旧する |
| 可聴グラフ | `graph` は**変化時のみ**なので、リレー側の `hears` は空のまま埋まらない |
| ブラウザ | 再接続して `resync` を受け取るが、**リレーが持っている中身が空** |
| 結果 | **誰かが動くまで無音** |

Rust のサーバーなら人は動き続けるので実際にはすぐ復旧するが、
**全員が拠点で静止している場面では止まったまま**になる。

既定 `5` 秒の代償は小さい。10 人で 60 バイト × 10 ÷ 5 秒 ≒ **120 B/s**。
「静止中に 2 Hz で流し続けない」という本来の意図は保たれる。

`0` にすると完全な変化検出になり、静止中のトラフィックはゼロになる。
上の復旧が要らない (リレーを再起動しない / 止まっても人が動くまで待てる) と
分かっている場合だけ。

> これは WS 接続時の撒き直しが無かったこと (issue #11) の回避策**ではない**。
> #11 はリレー側の `resync` で直っており、途中参加のブラウザは
> `hears_of` / `yaw_of` の中身を接続時に受け取る。上に書いたのは、
> **その `resync` が返す中身自体が空になる**という別の話。
