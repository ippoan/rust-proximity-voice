...
### `graph` は `peer` より先に来ることがある

リレー側で `Graph` は Hub へ直接、`Peer` は SFU のタスクを経由して出るので、
**接続時の撒き直し (issue #11) では `graph` のほうが先に届く**（実測: `graph → peer`）。
`graph` は「変化時のみ」なので、ここで取りこぼすと静止した場面では次が来ず、永久に無音になる。

`audio.js` は直近の `graph` を**全状態として保持**し、`peer` の割り当て時に引き直す。
`graph` は差分ではなく置き換えなので、載っていない相手は無音に戻る。
